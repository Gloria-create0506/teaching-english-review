...
如果你要发给其他老师、家长或学生一个真正可安装的 Windows 程序，直接使用构建好的安装包：

- `dist/Journey-Review-Setup.exe`

安装程序会：

- 把课件安装到当前用户目录 `%LOCALAPPDATA%\TeachingEnglishReview`
- 创建桌面快捷方式 `Journey Review Offline`
- 创建开始菜单文件夹 `Teaching English Review`
- 注册卸载入口，后续可以从开始菜单里的 `Uninstall Journey Review` 卸载

安装后的两种使用方式：

- `Journey Review Offline`：直接打开离线版网页课件，不依赖 Node.js
- `Journey Review LAN Server`：启动局域网服务端，供同一局域网其他设备访问；这一步需要电脑已安装 Node.js
...
如果你希望后续像普通网页一样持续更新，推荐使用 **Node.js 公网部署版**，不要继续反复发送安装包。
...
1. 把项目上传到一个 Git 仓库。
2. 在 `Render` 或 `Railway` 新建 Node Web Service。
3. 平台启动命令使用：
...
```text
ACCESS_PASSWORD=你的课堂密码
MAX_IPS_PER_PASSWORD=2
SESSION_HOURS=8
```

5. 部署完成后，把平台分配的网址发给学生或家长。
...
1. 更新 `chapter-*.html`
2. 更新对应的 `chapter-*-hero.png`
3. 如果新增章节入口，再修改 `chapters.html`
4. 重新部署到同一个公网项目

这样用户下次打开同一个网址时，看到的就是新版内容，不需要重新安装。

### Render 专用说明

项目已经带了 `render.yaml`，导入仓库后可以直接识别为 Node Web Service。

注意：

- 免费实例可能会休眠，第一次打开会慢几秒。
- 平台重启或重新部署后，`auth-state.json` 可能被清空。
- 这意味着“已绑定 IP 记录”可能会被重置，这是托管平台临时磁盘的正常现象。

如果你需要更稳定地保留 IP 绑定记录，应该改用：
...
如果你不想直接上传整个工作目录，可以生成一个更干净的公网部署包：
...
- `dist/Teaching-English-public-web.zip`
